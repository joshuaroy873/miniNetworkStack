...
EthernetFrame parse_ethernet_frame(ByteReader* p_reader) {
    // Parses an Ethernet frame from the reader buffer
    EthernetFrame frame;
    
    // dst_mac
    for (int i=0; i<MAC_ADDRESS_SIZE; i++) {frame.dst_mac[i] = read_u8(p_reader);}
    // src_mac
    for (int i=0; i<MAC_ADDRESS_SIZE; i++) {frame.src_mac[i] = read_u8(p_reader);}
    // ether_type
...
void print_mac(const uint8_t mac[MAC_ADDRESS_SIZE]) {
    for (int i = 0; i < MAC_ADDRESS_SIZE; i++) {
        std::cout << std::hex << std::setfill('0') << std::setw(2) << (int)mac[i];
        if (i < MAC_ADDRESS_SIZE-1) std::cout << ":";
    }
    std::cout << std::endl;
}
