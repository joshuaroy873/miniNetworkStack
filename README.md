Phase 1: Byte parsing (functions, no classes)
- write a function that reads a byte from an array
- write a function that reads 2 bytes in big-endian
- test with simple main()

Phase 2: Structs + simple parsing
- define EthernetFrame as a struct
- write simple parsing function (no error handling yet)
- print results

Phase 3: Error handling + vectors
- introduce safe bounds checking
- use vectors instead of raw arrays
- catch exceptions in main

Phase 4: Introduce ByteBuffer class
- now that you understand why, build the encapsulation
- understand move semantics from context

Phase 5: IPv4, UDP, etc.